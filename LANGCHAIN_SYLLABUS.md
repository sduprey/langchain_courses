# Building LLM Applications with LangChain & LangGraph (+ Git) — Syllabus & Agenda
...
This course is an **applied, frameworks-first** path to building LLM applications. After a
compact foundations block (how LLMs work, core prompting, reasoning and structured output),
it spends most of its time in **LangChain** (templates, parsers, memory, chains, RAG) and
**LangGraph** (tool calling, multi-step agents, memory), and closes on evaluation and safety.
**Git** is woven through as the workflow every lab and the capstone run on.

The hands-on labs use the notebooks in the companion repository — four mirrored
DeepLearning.AI short courses — plus an **Ollama recode** of the application-development
course so the fundamentals half runs free and offline. The material on Git, the foundations,
evaluation and security is additional content built for this course.

> A companion course, *Prompt & Context Engineering*, covers the foundations —
> evaluation, context engineering, cost and safety — in far more depth and on original,
> framework-independent material. Take it first if the goal is prompting rather than frameworks.
...
2. Explain how an LLM produces text and apply core prompting techniques.
3. Use reasoning strategies (chain-of-thought, self-consistency, ReAct) and produce
   validated structured outputs (JSON / schemas).
4. **Build LLM applications with LangChain**: prompt templates, output parsers, memory,
   chains and retrieval-augmented generation.
5. **Implement tool calling and multi-step agents with LangGraph**, including memory.
6. Evaluate LLM applications and defend them against prompt injection.
...
- **Track A — local (Ollama), free & offline.** Fundamentals half (Sessions 1–5). The
  ...
  material against **Ollama** instead of OpenAI, so students iterate with no API key, no cost,
...
> **Version note:** the mirrored DeepLearning.AI notebooks track a fast-moving `langchain`.
> Pin your dependencies and run every lab notebook once before class — import paths and
> deprecations shift between releases.
...
| # | Date | Time | Dur. | Git thread | Application-engineering thread |
|---|---|---|---|---|---|
...
Total Git time ≈ 4 h 40, front-loaded and returning for the capstone.
...
(models, prompt templates, output parsers, memory) · 1:00 lab (local, on a branch) · 1:40 wrap.

**S5 · 23 Sept (1h30).** 0:00 Git — keeping secrets out of the repo · 0:15 LangChain II —
chains & RAG · 0:40 lab: offline document QA + short quiz.
...
release · 1:20 consolidation (Git + application-engineering cheat sheets) · 1:35 where next
(CI/CD teaser) · 1:40 closing & resources.

## 8. Content beyond the repo (added for this course)

Git (the full thread above); the LLM mental model; the prompting-technique taxonomy; reasoning
strategies; structured outputs and validation; evaluation (eval sets, LLM-as-judge, regression);
prompt security (injection, jailbreaks, guardrails, PII); and the capstone framework and rubric.
...
- The companion course: *Prompt & Context Engineering* (original, framework-independent).
